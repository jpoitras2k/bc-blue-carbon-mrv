...
def load_and_validate_data(file_path="unified_bc_blue_carbon.csv", df=None):
    ...
    if not ((df['latitude'] >= 48.0) & (df['latitude'] <= 60.0)).all():
        ...
        print("Warning: " + str(len(out_of_range_lat)) + " latitudes are out of the 48.0-60.0 range. Samples:\n" + str(out_of_range_lat[['site_id', 'latitude', 'notes']].head()))
    if not ((df['longitude'] >= -140.0) & (df['longitude'] <= -114.0)).all():
        out_of_range_lon = df[(df['longitude'] < -140.0) | (df['longitude'] > -114.0)]
        print("Warning: " + str(len(out_of_range_lon)) + " longitudes are out of the -140.0--114.0 range. Samples:\n" + str(out_of_range_lon[['site_id', 'longitude', 'notes']].head()))

    # Validate categorical columns
    valid_regions = ['Salish Sea', 'West Coast VI', 'Central Coast', 'North Coast', 'Haida Gwaii', 'NE Pacific']
    valid_habitat_types = ['eelgrass', 'eelgrass_dwarf', 'kelp', 'saltmarsh', 'mixed_seagrass']
    valid_sediment_types = ['mud', 'sand', 'mud_sand', 'gravel', 'rocky', 'organic', 'unknown']
    valid_access_types = ['open', 'restricted', 'grey_literature', 'academic', 'estimated']

    invalid_regions = df[~df['region'].isin(valid_regions)]
    if not invalid_regions.empty:
        print(f"Warning: {len(invalid_regions)} regions are not in the valid list. Invalid values: {invalid_regions['region'].unique()}")
    
    invalid_habitat_types = df[~df['habitat_type'].isin(valid_habitat_types)]
    if not invalid_habitat_types.empty:
        print(f"Warning: {len(invalid_habitat_types)} habitat_types are not in the valid list. Invalid values: {invalid_habitat_types['habitat_type'].unique()}")
    ...
    if not invalid_sediment_types.empty:
        print(f"Warning: {len(invalid_sediment_types)} sediment_types are not in the valid list. Invalid values: {invalid_sediment_types['sediment_type'].unique()}")
    
    invalid_access_types = df[~df['access_type'].isin(valid_access_types)]
    if not invalid_access_types.empty:
        print(f"Warning: {len(invalid_access_types)} access_types are not in the valid list. Invalid values: {invalid_access_types['access_type'].unique()}")
    ...
    df['is_carbon_gap'] = df['notes'].fillna('').str.contains('CARBON_GAP:') | df['carbon_density_gCm2'].isna()
...
def process_janousek_data(file_path):
    ...
    df_janousek_seagrass['data_source'] = df_janousek_seagrass['data_source'].fillna('') + "; Janousek 2025 Data Ingest"
    ...
    expected_columns = ['site_id', 'site_name', 'latitude', 'longitude', 'region', 'habitat_type', 'sediment_type',
                        'carbon_density_gCm2', 'sequestration_rate_gCm2yr', 'measurement_depth_cm', 'survey_year',
                        'data_source', 'access_type', 'notes']
...
def process_hakai_data(file_path):
    ...
    print("Hakai df access_type immediately after read_csv:", df_hakai['access_type'].unique() if not df_hakai.empty else "Empty")
    ...
    df_hakai['data_source'] = df_hakai['data_source'].fillna('') + "; Hakai / Prentice 2020 Data Ingest"
...
def process_hakai_grain_size_data(file_path, df_unified):
    ...
    grain_size_regional_means = df_grain_size_bc_veg.groupby(['region'])[['percent_fines', 'percent_oc']].mean().reset_index()
    grain_size_regional_means.rename(columns={'percent_fines': 'regional_percent_fines_mean', 'percent_oc': 'regional_percent_oc_mean'}, inplace=True)

    # Calculate overall BC vegetated mean for imputation fallback
    bc_vegetated_overall_mean_fines = 1.92 # Provided in the task
    bc_vegetated_overall_mean_oc = df_grain_size_bc_veg['percent_oc'].mean() # Dynamically calculate

    # Merge into the unified dataframe based on site and region
    df_unified = pd.merge(df_unified, grain_size_site_means, how='left', left_on=['site_name', 'region'], right_on=['site', 'region'], suffixes=('', '_grain_size'))
    if 'site_grain_size' in df_unified.columns:
        df_unified.drop(columns=['site_grain_size'], inplace=True) # Drop the redundant site column
    ...
    bc_regions = ['Salish Sea', 'West Coast VI', 'Central Coast', 'North Coast', 'Haida Gwaii', 'NE Pacific']
    vegetated_habitat_types = ['eelgrass', 'eelgrass_dwarf', 'saltmarsh', 'mixed_seagrass']
    is_bc_vegetated_site = df_unified['region'].isin(bc_regions) & df_unified['habitat_type'].isin(vegetated_habitat_types)
    ...
    if missing_fines_mask.any():
        ...
        df_unified = pd.merge(df_unified, grain_size_regional_means, how='left', on='region', suffixes=('', '_regional_mean'))
        df_unified.loc[missing_fines_mask, 'percent_fines'] = df_unified.loc[missing_fines_mask, 'regional_percent_fines_mean'].fillna(bc_vegetated_overall_mean_fines)
    ...
    if missing_oc_mask.any():
        # First try to impute with regional means
        if 'regional_percent_oc_mean' not in df_unified.columns: # Ensure it's there if not merged for fines
            df_unified = pd.merge(df_unified, grain_size_regional_means, how='left', on='region', suffixes=('', '_regional_mean'))
        df_unified.loc[missing_oc_mask, 'percent_oc'] = df_unified.loc[missing_oc_mask, 'regional_percent_oc_mean'].fillna(bc_vegetated_overall_mean_oc)
    ...
    df_unified['percent_fines'] = df_unified['percent_fines'].fillna(df_unified['percent_fines'].mean())
    df_unified['percent_oc'] = df_unified['percent_oc'].fillna(df_unified['percent_oc'].mean())
...
def process_hakai_isotope_data(file_path, df_unified):
    ...
    df_isotope.rename(columns={'site': 'site_name', 'avg_d13C': 'd13C', 'avg_d15N': 'd15N'}, inplace=True)
    ...
    df_unified = pd.merge(df_unified, df_isotope[['site_name', 'd13C', 'd15N']], how='left', on=['site_name'], suffixes=('', '_isotope'))
...
def engineer_features(df):
    ...
    df['min_distance_to_port_km'] = df.apply(lambda row:
        min([haversine_distance(row['latitude'], row['longitude'], port_coords[0], port_coords[1])
             for port_coords in ports.values()]) if pd.notna(row['latitude']) and pd.notna(row['longitude']) else np.nan,
        axis=1
    )
    # Inverse of distance as a simple stress index (closer = higher stress)
    df['anthropogenic_stress_index'] = 1 / (df['min_distance_to_port_km'] + 1) # Add 1 to avoid division by zero

    # Impute remaining NaNs in all numerical features before model selection
    # This step is crucial to ensure models like GradientBoostingRegressor don't receive NaNs.
    numerical_features_to_impute_final = [
        'latitude', 'longitude', 'habitat_area_ha', 'anthropogenic_stress_index',
        'percent_fines', 'percent_oc', 'd13C', 'd15N', 'measurement_depth_cm', 'survey_year'
    ]
    ...
    features = [col for col in df.columns if col.startswith(('latitude', 'longitude', 'region_', 'habitat_type_', 'habitat_area_ha', 'anthropogenic_stress_index', 'percent_fines', 'percent_oc', 'd13C', 'd15N'))]
...
def train_and_predict_model(features_df, df_engineered):
    ...
    if not predict_df.empty:
        ...
        df_engineered.loc[df_engineered['is_carbon_gap'], 'predicted_carbon_density_gCm2'] = model.predict(X_predict)
    else:
        df_engineered['predicted_carbon_density_gCm2'] = np.nan
...
if __name__ == "__main__":
    ...
    df_combined = pd.concat([df, crd_df, processed_janousek_df, processed_hakai_df], ignore_index=True)
    ...
    df_combined = process_hakai_grain_size_data("data/hakai_grain_size.csv", df_combined)
    ...
    y_train_for_importance = train_df_for_importance['carbon_density_gCm2'].astype(float)
